...
#   --norm-scope global|fold_trainval|fold_train    (default: global)
# Windows-safe DataLoader fallback. AMP-safe. Resuming supported.
# ------------------------------------------------------------
...
def build_loaders(train_df, val_df, out_hw=(256,256), bs=2, num_workers=4, mean=(0.5,0.5,0.5), std=(0.5,0.5,0.5),
    ...
    """Windows-safe defaults; on Windows force workers=0 to avoid spawn/pickle crashes."""
...
def build_loaders_with_fallback(train_df, val_df, *, out_hw, bs, num_workers, mean, std, device, prefetch_factor):
    """Try to build with requested num_workers; if first batch fails, rebuild with workers=0."""
    def _make(nw):
...
def estimate_global_mean_std(df_all, out_hw, sample_n=5000, cache_path: Path | None = None):
    """Compute μ,σ from a random sample; optionally cache to disk."""
    if cache_path and cache_path.exists():
        try:
            d = json.loads(cache_path.read_text())
            m = np.array(d["mean"], dtype=np.float32); s = np.array(d["std"], dtype=np.float32)
            return m, s, True
        except Exception:
            pass
    if len(df_all) == 0:
        m = np.array([0.5,0.5,0.5], np.float32); s = np.array([0.5,0.5,0.5], np.float32)
        if cache_path: cache_path.write_text(json.dumps(dict(mean=m.tolist(), std=s.tolist())))
        return m, s, False
    # random sample of windows (uniform over rows)
    rng = np.random.default_rng(123)
    idxs = rng.choice(len(df_all), size=min(sample_n, len(df_all)), replace=False)
    sampled = df_all.iloc[np.sort(idxs)]
    m, s = estimate_mean_std(sampled, n=len(sampled), out_hw=out_hw, apply_sky=False)
    if cache_path:
        try:
            cache_path.write_text(json.dumps(dict(mean=m.tolist(), std=s.tolist())))
        except Exception:
            pass
    return m, s, False
...
def main():
    ...
    ap.add_argument("--norm-scope", type=str, default="global",
                    choices=["global", "fold_trainval", "fold_train"],
                    help="global=single μ,σ for all folds; fold_trainval=train∪val per fold; fold_train=train only per fold")
    ap.add_argument("--norm-sample", type=int, default=5000,
                    help="number of windows to sample when computing global μ,σ")
    args = ap.parse_args()
    ...
    # Global normalization (if selected)
    global_mean, global_std = None, None
    if args.norm_scope == "global":
        cache = out_root / "global_norms.json"
        global_mean, global_std, from_cache = estimate_global_mean_std(
            asi_seq, out_hw=OUT_HW, sample_n=args.norm_sample, cache_path=cache
        )
        tag = "(cached)" if from_cache else "(new)"
        print(f"[NORM][global] mean={np.round(global_mean,4)} std={np.round(global_std,4)} {tag}")
    ...
    for (train_seq, val_seq, info) in splits:
        ...
        # Choose μ,σ according to strategy
        if args.norm_scope == "global":
            mean, std = global_mean, global_std
        elif args.norm_scope == "fold_trainval":
            tv = pd.concat([train_seq, val_seq], axis=0)
            mean, std = estimate_mean_std(tv, out_hw=OUT_HW, apply_sky=False)
        else:  # fold_train
            mean, std = estimate_mean_std(train_seq, out_hw=OUT_HW, apply_sky=False)
        mean = np.asarray(mean, dtype=np.float32); std = np.asarray(std, dtype=np.float32)
        print(f"[NORM][fold {fold_id}] scope={args.norm_scope} | mean={np.round(mean,4)} std={np.round(std,4)}")
        ...
        if args.resume_path is None:
            ...
            args.resume_path = None  # consume it once
